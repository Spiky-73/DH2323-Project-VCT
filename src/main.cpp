...
PointLight light{ glm::vec3(0, 0.3, -1.1), glm::vec3(1,1,1), 14 };
...
const int SHADOW_MAPS_RESOLUTION = 256;
float shadowMaps[ShadowCube::FACES][SHADOW_MAPS_RESOLUTION * SHADOW_MAPS_RESOLUTION];
ShadowCube shadowCube{
	&light.position,
	shadowMaps[0], shadowMaps[1], shadowMaps[2], shadowMaps[3], shadowMaps[4], shadowMaps[5],
	SHADOW_MAPS_RESOLUTION,
};
...
void SetupShaders() {
	...
	renderShader.shadowCube = &shadowCube;
...
void Draw()
{
	...
	Camera lightCamera = { light.position, glm::mat3(), glm::ivec2(SHADOW_MAPS_RESOLUTION, SHADOW_MAPS_RESOLUTION), SHADOW_MAPS_RESOLUTION/2 };
	shadowShader.camera = &lightCamera;
	// for (size_t f = 0; f < ShadowCube::FACES; f++) {
	{
		size_t f =0;
		lightCamera.rotation = ShadowCube::rotations[f];
		shadowShader.depthBuffer = shadowMaps[f];
		lab3Shader.camera = &lightCamera;
		for (size_t i = 0; i < SHADOW_MAPS_RESOLUTION * SHADOW_MAPS_RESOLUTION; i++) shadowMaps[f][i] = 0;
		for (const auto& t : triangles) {
			lab3Shader.normal = t.normal;
			lab3Shader.reflectance = t.color;
			renderer.DrawPolygon({ {t.v0}, {t.v1}, {t.v2} }, lab3Shader, DrawMode::Default);
		}
	}

...
Camera camera{ glm::vec3(0,0,-3.001), glm::mat3(1, 0, 0, 0, 1, 0, 0, 0, 1), RESOLUTION_Y };
...
// ----------------------------------------------------------------------------
// SHADERS
RenderMode renderMode = RenderMode::VCT;
...
Light ambiantLight{ glm::vec3(), glm::vec3(1, 1, 1), 0.5f };
float depthBuffer[RESOLUTION_Y * RESOLUTION_X];
const int VOXEL_RESOLUTION = 32;
glm::vec4 voxels[VOXEL_RESOLUTION * VOXEL_RESOLUTION * VOXEL_RESOLUTION];
...
void SetupShaders() {
	...
	lab3Shader.resolution = glm::ivec2(RESOLUTION_X, RESOLUTION_Y);
	...
	voxelRenderShader.resolution = glm::ivec2(RESOLUTION_X, RESOLUTION_Y);
	...
	renderShader.resolution = glm::ivec2(RESOLUTION_X, RESOLUTION_Y);
...
void Update(void) {
	...
	if (keystate[SDL_SCANCODE_3]) renderMode = RenderMode::ShadedVoxels;
	if (keystate[SDL_SCANCODE_4]) renderMode = RenderMode::VCT;
	...
	for (size_t x = 0; x < VOXEL_RESOLUTION; x++) {
		for (size_t y = 0; y < VOXEL_RESOLUTION; y++) {
			for (size_t z = 0; z < VOXEL_RESOLUTION; z++) {
				if (voxels[x + VOXEL_RESOLUTION * y + VOXEL_RESOLUTION * VOXEL_RESOLUTION * z].a != 0) {
					glm::vec3 anchor = bboxMin + vs * glm::vec3(x, y, z);
					voxelRenderShader.reflectance = glm::vec3(voxels[x + VOXEL_RESOLUTION * y + VOXEL_RESOLUTION * VOXEL_RESOLUTION * z]);
...
void Draw()
{
	...
	// 1. Generated the voxels with the reflectance as the color by "rendering the scene" on an orthographic camera centered on one side of the grid
	for (size_t i = 0; i < VOXEL_RESOLUTION * VOXEL_RESOLUTION * VOXEL_RESOLUTION; i++) voxels[i].a = 0;
	for (const auto& t : triangles) {
	...
	// 2. Bake in the visibility by ray casting from each voxel to light
	...
	for (const auto& t : triangles) {
		...
		renderer.DrawPolygon({ {t.v0}, {t.v1}, {t.v2} }, shadowShader, mode);
	}

	if (renderMode == RenderMode::ShadedVoxels) {
		DrawVoxels();
		sdlAux->render();
		return;
	}
